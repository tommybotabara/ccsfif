...
<?php
		...
		{
		$ID = $_POST['chosen'];
		$q = "SELECT id, name ,hours, termid,schoolyearid FROM workload where id = '".$ID."'";
		...
		if (isset($_POST['Submit']))
		{
			...
			if(!isset($message))
			{
				$query="insert into workload (id,name,hours,fid,termid,schoolyearid) values ('{$ctr}','{$_POST["workload"]}','{$_POST["hours"]}','{$fidd}','{$_POST["termchoose"]}','{$_POST["sychoose"]}')";
          ...
          <div class="col-lg-12">
            ...
            <div class="form-horizontal style-form">
                ...
                <div class="form-group">
                  <label class="col-sm-2 col-sm-2 control-label">Workload/Activity Name</label>
                  <div class="col-sm-8">
                    <input type="text" class="form-control" name="workload" value ="<?php echo $wlname ?>">
                  </div>
                </div> 
                    
                <div class="form-group">
                  <label class="col-sm-2 col-sm-2 control-label">Hours (per week)</label>
                  <div class="col-sm-2">
                    <input type="number" step="0.1" class="form-control" name="hours" value ="<?php echo $wlhours ?>">
                  </div>
                </div> 
                ...
                <div class="form-group">
					...
					<div class="col-sm-2">
						...
						while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))
						{
						  ...
						  if($row['syid'] == $wlschoolyear)
								{
									echo 'selected="selected "';
								}
						  echo"value={$row['syid']}>".$row['schoolyearstart']."-".$row['schoolyearend']."</option>";
						}
						?>
						</select>
					</div>
					...
					<div class="col-sm-2">
						...
						{
						  echo "<option ";
						  if($row['termid'] == $wltermid)
								{
									echo 'selected="selected "';
								}
						  echo "value={$row1['termid']}>".$row1['term']."</option>";
						}
						?>
						</select>
					</div>
					...
					<button class="btn btn-theme" name="Submit" type="Submit" value="Submit" onclick="return confirm('Are you sure?')">Update </button>
					</div>
                    </div>
                    </form>
